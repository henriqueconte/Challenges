...
class Solution:
    ...
    def orangesRotting(self, grid):
        ...
        while q:
            i, j, rotten_minutes = q.popleft()

            if i < len(grid) - 1 and grid[i+1][j] == 1:
                grid[i+1][j] = 2
                q.append([i+1,j,rotten_minutes+1])

            if j < len(grid[0]) - 1 and grid[i][j+1] == 1:
                grid[i][j+1] = 2
                q.append([i,j+1,rotten_minutes+1])
        ...
        if total_orange_count == total_rotten_count:
            return rotten_minutes
        else:
            return -1
